...
FabricClient.newDefaultKeyValueStore({ path: storePath
  ...
  console.log('Instantiating chaincode ' + ccName + ':' + ccVersion + ' on channel ' + channel.getName() + '...');
  return channel.sendInstantiateProposal(instantiateChaincodeRequest);
}).then((results) => {
	...
	if (isProposalGood) {
		...
		var transactionIdString = txId.getTransactionID(); //Get the transaction ID string to be used by the event processing
		var promises = [];
